...
jQuery(document).ready(function ($) {
	...
	*/
	...
	function initIsotopeFiltering() {
		...
		if ($('.product-grid').length) {
			...
			// Lấy tham số tìm kiếm hiện có trên URL
			var urlParams = new URLSearchParams(location.search);

			// Xử lý sự kiện khi người dùng nhấn nút lọc sản phẩm theo khoảng giá
			filterButton.on('click', function () {
				// Lấy giá trị phạm vi giá từ phần tử #amount
				var priceRange = $('#amount').val();
				// Tách giá trị tối thiểu và tối đa từ phạm vi giá và bỏ ký tự đơn vị tiền tệ ($)
				var priceMin = parseFloat(priceRange.split('-')[0].replace('đ', ''));
				var priceMax = parseFloat(priceRange.split('-')[1].replace('đ', ''));
				// Tạo URL mới với các tham số tìm kiếm hiện có và thêm tham số tìm kiếm mới cho giá
				var url = "?minPrice=" + priceMin + "&maxPrice=" + priceMax;
				urlParams.forEach(function (value, key) {
					if (key !== 'minPrice' && key !== 'maxPrice') {
						url += '&' + key + '=' + value;
					}
				});
				// Chuyển hướng đến URL mới để hiển thị sản phẩm đã được lọc theo giá
	...
	*/
	...
	function initPriceSlider() {
		...
		// Kiểm tra nếu có giá trị của minPrice và maxPrice trong URL thì sử dụng giá trị này để thiết lập lại thanh trượt
